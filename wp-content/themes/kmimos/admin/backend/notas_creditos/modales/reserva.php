...
<?php
	...
	if( $pedido_id > 0 && date('YmdHis',strtotime($reserva_end)) <= date('YmdHis') ){
		$pedido_id = 0;
 	}
...
<?php if( !$show_nc ){ ?>
	<input type="text" value="<?php print_r("SELECT post_parent FROM wp_posts where ID = {$ID} and post_status in ( 'confirmed', 'complete' )")?>">
